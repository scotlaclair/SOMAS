...
| **The State** | Labels | `somas:stage:implement`, `somas:status:blocked`, `somas:gate:passed`. Triggers workflow transitions. |
...
|----|----------------|--------------------------|--------------|---------------------|---------------------|
| 01 | **INTAKE** | triage, advisor | `somas:stage:intake` | Agent analyzes Issue body. Checks similarity to existing issues. Assigns difficulty. | `feasibility > 0.7` |
| 02 | **SPECIFY** | specifier | `somas:stage:specify` | Agent creates/updates `docs/specs/ISSUE-123.md`. Converts intent to strict PRD. | `spec_approved` |
| 03 | **PLAN** | planner, architect, simulator | `somas:stage:plan` | Architect defines folder structure. Planner generates `plan.json`. Simulator runs Monte Carlo pathing. | `architecture_valid` |
| 04 | **DECOMPOSE** | decomposer | `somas:stage:decompose` | Breaks `plan.json` into atomic sub-tasks linked to the Issue. | `tasks_created` |
| 05 | **IMPLEMENT** | implementer, copilot | `somas:stage:implement` | Create PR. Loop: Write Code → Run CI → Fix Errors. (See Micro-Loop below) | `build_success` |
| 06 | **VERIFY** | tester, debugger | `somas:stage:verify` | Runs deep test suites on the PR. Debugger patches logic if tests fail. | `tests_pass` |
| 07 | **INTEGRATE** | merger, validator | `somas:stage:integrate` | Checks for merge conflicts with main. Validates requirements traceability. | `clean_merge` |
| 08 | **HARDEN** | security | `somas:stage:harden` | Runs SAST/DAST on PR. Checks for secrets/vulns. | `security_clean` |
| 09 | **RELEASE** | deployer | `somas:stage:release` | Merges PR to main. Tags release. Generates Changelog. | `merged` |
| 10 | **OPERATE** | operator | `somas:stage:operate` | Deploys to env. Checks health endpoints. | `health_200_ok` |
| 11 | **ANALYZE** | analyzer, documenter | `somas:stage:analyze` | Reviews metrics (time-to-merge, retries). Updates strictness of Gates. | `metrics_logged` |
...
While the Issue is in the IMPLEMENT column (`somas:stage:implement`), the PR undergoes its own cycle before moving to Stage 06:
...
7. **Next:** Move to VERIFY (`somas:stage:verify`) for functional correctness testing.
...
The **Orchestrator** (`orchestrator.yml`) is the Runtime Engine. It does not appear in the table because it *is* the table. It manages the movement of the Issue card between columns and invokes the Agents assigned to that column based on the presence of labels like `somas:stage:*`.
